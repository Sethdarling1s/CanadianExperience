            ...
            wxBORDER_SIMPLE)
{
	...
	parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewTimeline::SelectMachineOne, this, XRCID("MachineDialog"));
	parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewTimeline::SelectMachineTwo, this, XRCID("Machine2Dialog"));
	parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewTimeline::OnSetMachineStart, this, XRCID("MachineStart"));
...
/**
 * Handles opening Machine Start Dialog box
 * @param event wxCommandEvent to be handled
 */
void ViewTimeline::OnSetMachineStart(wxCommandEvent& event)
{
	MachineStartDlg dlg(this->GetParent(), GetPicture()->GetMachineOne(), GetPicture()->GetMachineTwo());
...
/**
 * Brings up dialog box to select machine
 * @param event Machine Select event object
 */
void ViewTimeline::SelectMachineOne(wxCommandEvent& event)
{
	shared_ptr<MachineDialog> dlg = (GetPicture()->GetMachineOne()->CreateDialog(GetParent()));
	if (dlg->ShowModal() == wxID_OK)
	{
 ...
 * @param event Machine Select event object
 */
void ViewTimeline::SelectMachineTwo(wxCommandEvent& event)
{
	shared_ptr<MachineDialog> dlg = (GetPicture()->GetMachineTwo()->CreateDialog(GetParent()));
	if (dlg->ShowModal() == wxID_OK)
	{
		// A machine has been selected
	}
