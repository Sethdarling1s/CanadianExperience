 ...
 */
MachineOneFactory::MachineOneFactory(std::wstring resourcesDir, ma_engine* audioEngine) :
 ...
 */
std::shared_ptr<Machine> MachineOneFactory::CreateMachineOne()
{
	...
	cardReader->SetCard(mResourcesDir + L"/images/cards/suzanna.png");
	...
	drum->SetLocation(wxPoint(CymbalAndDrumCenter, -100));




	auto drumCylinder = std::make_shared<Cylinder>(mResourcesDir, 44, .25);
	drumCylinder->SetLocation(wxPoint(110, -30));
	...
	// The cymbal
	//
	auto cymbal = std::make_shared<StruckInstrument>(mResourcesDir, mAudioEngine, L"/audio/cymbal.wav", L"/images/cymbal-all.png");
	...
	cymbalCylinder->SetLocation(wxPoint(CymbalAndDrumCenter - 25, -350));
	...
	for (int i = 0; i < 5; i++)
	{
		...
		tubing->AddClamp(wxPoint(ClampingPostCenter, -10 - 10*i), 40, .75);
	...
	tubing->AddClamp(wxPoint(ClampingPostCenter, -10 - 10 * 5), 50, .75);
	...
	clamps->SetLocation(wxPoint(ClampingPostCenter, 0));
